...
# Logs visibles en consola (Windows / Cursor a veces no muestran solo uvicorn)
...
app = FastAPI(
    title="FitLife Backend API",
    description="Hexagonal Architecture Backend for FitLife App",
    version="0.1.0",
)
...
def _log_to_file(msg: str):
    """Escribe en archivo para verificar que las peticiones llegan (Windows a veces no muestra consola)."""
    try:
        # Log en la carpeta del backend (donde está run.py)
...
async def log_requests(request, call_next):
    """Log cada request para depuración (ver si llegan las peticiones)."""
    import time
    start = time.time()
...
# Archivos estáticos (certificados subidos)
...
async def test_email(email: str = ""):
    """Prueba SMTP. URLs: http://localhost:8000/test-email?email=tu@email.com"""
    ...
    try:
        ...
        sent = await SmtpEmailService.send_password_reset_email(
            to_email=email, reset_token="test-token-123", user_name="Test"
        )
