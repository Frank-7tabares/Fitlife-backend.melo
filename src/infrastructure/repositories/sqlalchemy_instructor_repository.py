"""Repositorios para instructores, asignaciones y calificaciones."""
...
class SQLAlchemyInstructorRepository:
    ...
    async def save(self, instructor: Instructor) -> Instructor:
        model = InstructorModel(
            ...
            certificate_url=getattr(instructor, "certificate_url", None),
            certificate_status=getattr(instructor, "certificate_status", "pending"),
        )
    ...
    async def delete_by_id(self, instructor_id: UUID) -> bool:
        """Elimina un instructor por ID. Usar antes de eliminar el usuario."""
    ...
    async def update_certificate_status(self, instructor_id: UUID, status: str) -> None:
        stmt = (
            update(InstructorModel)
            .where(InstructorModel.id == str(instructor_id))
            .values(certificate_status=status)
        )
    ...
    async def update_rating_avg(self, instructor_id: UUID, new_avg: float) -> None:
        stmt = (
            update(InstructorModel)
            .where(InstructorModel.id == str(instructor_id))
            .values(rating_avg=new_avg)
        )
        await self.session.execute(stmt)
        await self.session.commit()

    async def find_active_assignment(self, user_id: UUID) -> Optional[InstructorAssignment]:
        """Asignación instructor–atleta activa para un usuario (atleta). Usado en mensajería."""
...
class SQLAlchemyInstructorAssignmentRepository:
    ...
    async def save(self, assignment: InstructorAssignment) -> InstructorAssignment:
        model = InstructorAssignmentModel(
    ...
    async def deactivate_active_for_user(self, user_id: UUID) -> None:
        stmt = (
            update(InstructorAssignmentModel)
            .where(
                InstructorAssignmentModel.user_id == str(user_id),
                InstructorAssignmentModel.is_active == True,
            )
    ...
    async def find_active_by_user(self, user_id: UUID) -> Optional[InstructorAssignment]:
        ...
        return InstructorAssignment(
            ...
            instructor_id=UUID(m.instructor_id),
    ...
    async def exists_active_pair(self, instructor_id: UUID, client_user_id: UUID) -> bool:
        """True si el instructor tiene al atleta asignado activamente."""
        stmt = (
            select(InstructorAssignmentModel.id)
            .where(
                InstructorAssignmentModel.instructor_id == str(instructor_id),
                InstructorAssignmentModel.user_id == str(client_user_id),
                InstructorAssignmentModel.is_active == True,
            )
            .limit(1)
        )
    ...
    async def count_active_by_instructor(self, instructor_id: UUID) -> int:
        stmt = (
            select(func.count())
            .select_from(InstructorAssignmentModel)
            .where(
                InstructorAssignmentModel.instructor_id == str(instructor_id),
                InstructorAssignmentModel.is_active == True,
            )
...
class SQLAlchemyInstructorRatingRepository:
    ...
    async def save(self, rating: InstructorRating) -> InstructorRating:
        model = InstructorRatingModel(
            id=str(rating.id),
            user_id=str(rating.user_id),
            instructor_id=str(rating.instructor_id),
    ...
    async def get_average_rating(self, instructor_id: UUID) -> Optional[float]:
        stmt = (
            select(func.avg(InstructorRatingModel.rating))
            .where(InstructorRatingModel.instructor_id == str(instructor_id))
        )
        result = await self.session.execute(stmt)
        avg = result.scalar()
        if avg is None:
