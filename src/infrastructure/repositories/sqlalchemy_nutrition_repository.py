...
class SQLAlchemyNutritionRepository:
    ...
    async def save(self, plan: NutritionPlan) -> NutritionPlan:
        ...
        if plan.is_active:
            stmt = update(NutritionPlanModel).where(
                NutritionPlanModel.user_id == str(plan.user_id),
    ...
    async def delete_plans_by_user_id(self, user_id: UUID) -> int:
        """Elimina todos los planes nutricionales de un usuario. Devuelve la cantidad eliminada."""
    ...
    async def find_active_by_user_id(self, user_id: UUID) -> Optional[NutritionPlan]:
        stmt = select(NutritionPlanModel).where(
            NutritionPlanModel.user_id == str(user_id),
            NutritionPlanModel.is_active == True
        )
        result = await self.session.execute(stmt)
