"""Repositorio de historial de contraseñas usando SQLAlchemy."""
...
class SQLAlchemyPasswordHistoryRepository(PasswordHistoryRepository):
    """Persiste y consulta historial de contraseñas."""
    ...
    async def save(self, history: PasswordHistory) -> None:
        ...
        model = PasswordHistoryModel(
            ...
            password_hash=history.password_hash,
    ...
    async def get_last_n_hashes(self, user_id: UUID, n: int = 5) -> list[str]:
        ...
        stmt = (
            ...
            .where(PasswordHistoryModel.user_id == str(user_id))
            .order_by(desc(PasswordHistoryModel.changed_at))
            .limit(n)
        )
    ...
    async def delete_old_entries(self, user_id: UUID, keep: int = 5) -> None:
        """Elimina entradas antiguas, manteniendo los últimos n registros."""
        # Obtener los IDs de los últimos n registros a mantener
        stmt = (
        ...
        delete_stmt = delete(PasswordHistoryModel).where(
            PasswordHistoryModel.user_id == str(user_id),
            ~PasswordHistoryModel.id.in_(ids_to_keep) if ids_to_keep else True,
        )
