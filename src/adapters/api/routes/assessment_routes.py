...
async def submit_assessment(request: SubmitAssessmentRequest, repo: SQLAlchemyAssessmentRepository=Depends(get_assessment_repository), user_repo: SQLAlchemyUserRepository=Depends(get_user_repository)):
...
async def get_history(user_id: UUID, repo: SQLAlchemyAssessmentRepository=Depends(get_assessment_repository)):
