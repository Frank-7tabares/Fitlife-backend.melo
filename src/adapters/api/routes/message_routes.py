...
from src.application.use_cases.message_use_cases import SendMessage, GetMessagesByUser, MarkMessageAsRead, GetConversation, MarkThreadRead, GetCoachInbox
from src.application.dtos.message_dtos import SendMessageRequest, MessageResponse, MessageListResponse, ConversationResponse, CoachInboxResponse
...
async def send_message(request: SendMessageRequest, current_user: User=Depends(get_current_user), message_repo: SQLAlchemyMessageRepository=Depends(get_message_repo), user_repo: SQLAlchemyUserRepository=Depends(get_user_repo), instructor_repo: SQLAlchemyInstructorRepository=Depends(get_instructor_repo)):
...
async def get_coach_inbox(current_user: User=Depends(get_current_user), message_repo: SQLAlchemyMessageRepository=Depends(get_message_repo), user_repo: SQLAlchemyUserRepository=Depends(get_user_repo)):
...
async def get_user_messages(user_id: UUID, current_user: User=Depends(get_current_user), skip: int=0, limit: int=50, message_repo: SQLAlchemyMessageRepository=Depends(get_message_repo)):
...
async def get_conversation(peer_id: UUID, current_user: User=Depends(get_current_user), message_repo: SQLAlchemyMessageRepository=Depends(get_message_repo), user_repo: SQLAlchemyUserRepository=Depends(get_user_repo), instructor_repo: SQLAlchemyInstructorRepository=Depends(get_instructor_repo), assignment_repo: SQLAlchemyInstructorAssignmentRepository=Depends(get_assignment_repo)):
...
async def mark_conversation_read(peer_id: UUID, current_user: User=Depends(get_current_user), message_repo: SQLAlchemyMessageRepository=Depends(get_message_repo), user_repo: SQLAlchemyUserRepository=Depends(get_user_repo), instructor_repo: SQLAlchemyInstructorRepository=Depends(get_instructor_repo), assignment_repo: SQLAlchemyInstructorAssignmentRepository=Depends(get_assignment_repo)):
...
async def mark_message_as_read(message_id: UUID, current_user: User=Depends(get_current_user), message_repo: SQLAlchemyMessageRepository=Depends(get_message_repo)):
