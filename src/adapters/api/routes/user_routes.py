...
from src.application.dtos.user_dtos import UpdateProfileRequest, UserProfileResponse, ProfileAuditHistoryResponse, ActiveInstructorResponse
...
from src.application.use_cases.message_use_cases import resolve_coach_user_id_for_profile, coach_owns_assignment
...
async def get_my_active_instructor(current_user: User=Depends(get_current_user), db: AsyncSession=Depends(get_db)):
    ...
    if asn:
        ...
        messaging_uid = await resolve_coach_user_id_for_profile(asn.instructor_id, user_repo, inst_repo)
        return ActiveInstructorResponse(instructor_id=asn.instructor_id, instructor_name=name, messaging_user_id=messaging_uid)
    ...
    for msg in recent_msgs:
        ...
        if sender and sender.role == UserRole.INSTRUCTOR:
            ...
            return ActiveInstructorResponse(instructor_id=sender.id, instructor_name=name, messaging_user_id=sender.id)
    return ActiveInstructorResponse()

@router.get('/clients', response_model=List[AdminUserListItem])
async def list_clients(_current_user=Depends(get_current_instructor), user_repo: SQLAlchemyUserRepository=Depends(get_user_repository)):
    users = await user_repo.find_by_role(UserRole.USER.value)
    return [AdminUserListItem(id=u.id, email=u.email, full_name=u.full_name, role=u.role, is_active=u.is_active, created_at=u.created_at) for u in users]

@router.get('/clients/assigned', response_model=List[AdminUserListItem])
async def list_assigned_clients(current_user: User=Depends(get_current_user), user_repo: SQLAlchemyUserRepository=Depends(get_user_repository), db: AsyncSession=Depends(get_db)):
    ...
    inst_repo = SQLAlchemyInstructorRepository(db)
    stmt = select(InstructorAssignmentModel.user_id, InstructorAssignmentModel.instructor_id).where(InstructorAssignmentModel.is_active.is_(True))
    ...
    for pid in profile_ids:
        if str(pid).lower() == coach_key:
            ...
            owns_cache[pid] = await coach_owns_assignment(pid, current_user.id, user_repo, inst_repo)
    seen: set[UUID] = set()
    user_ids: List[UUID] = []
    ...
    for uid in user_ids:
        ...
        if u:
            out.append(AdminUserListItem(id=u.id, email=u.email, full_name=u.full_name, role=u.role, is_active=u.is_active, created_at=u.created_at))
    return out
...
async def update_profile(user_id: UUID, request: UpdateProfileRequest, repos: tuple=Depends(get_repos)):
...
async def get_user_assessments(user_id: UUID, repo: SQLAlchemyAssessmentRepository=Depends(get_assessment_repository)):
    use_case = GetAssessmentHistory(repo)
    return await use_case.execute(user_id)

@router.post('/{user_id}/assign-instructor')
async def assign_instructor(user_id: UUID, request: AssignInstructorRequest, use_cases: InstructorUseCases=Depends(get_instructor_use_cases)):
    try:
