...
async def create_reminder(user_id: UUID, request: CreateReminderRequest, reminder_repo: SQLAlchemyReminderRepository=Depends(get_reminder_repo), user_repo: SQLAlchemyUserRepository=Depends(get_user_repo)):
...
async def get_user_reminders(user_id: UUID, skip: int=0, limit: int=50, reminder_repo: SQLAlchemyReminderRepository=Depends(get_reminder_repo)):
    use_case = GetRemindersByUser(reminder_repo)
    return await use_case.execute(user_id, skip, limit)

@router.get('/{reminder_id}', response_model=ReminderResponse)
async def get_reminder(reminder_id: UUID, reminder_repo: SQLAlchemyReminderRepository=Depends(get_reminder_repo)):
...
async def update_reminder(reminder_id: UUID, request: UpdateReminderRequest, reminder_repo: SQLAlchemyReminderRepository=Depends(get_reminder_repo)):
...
async def delete_reminder(reminder_id: UUID, reminder_repo: SQLAlchemyReminderRepository=Depends(get_reminder_repo)):
