...
async def create_plan(user_id: UUID, request: CreateNutritionPlanRequest, _current_user=Depends(get_current_instructor), repo: SQLAlchemyNutritionRepository=Depends(get_repo)):
...
async def get_active_plan(user_id: UUID, repo: SQLAlchemyNutritionRepository=Depends(get_repo)):
    use_cases = NutritionUseCases(repo)
