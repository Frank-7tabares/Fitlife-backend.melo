...
async def list_routines_by_creator(creator_id: UUID=Query(..., description='ID del instructor que creó las rutinas'), current_user: User=Depends(get_current_instructor), use_cases: TrainingUseCases=Depends(get_use_cases)):
...
async def create_routine(request: CreateRoutineRequest, current_user: User=Depends(get_current_instructor), use_cases: TrainingUseCases=Depends(get_use_cases)):
...
async def update_routine(routine_id: UUID, request: UpdateRoutineRequest, current_user: User=Depends(get_current_instructor), use_cases: TrainingUseCases=Depends(get_use_cases)):
    try:
...
async def delete_routine(routine_id: UUID, current_user: User=Depends(get_current_instructor), use_cases: TrainingUseCases=Depends(get_use_cases)):
    try:
...
async def assign_routine(user_id: UUID, request: AssignRoutineRequest, _current_user: User=Depends(get_current_instructor), use_cases: TrainingUseCases=Depends(get_use_cases)):
...
async def get_user_completions(user_id: UUID, limit: int=Query(50, ge=1, le=100), current_user: User=Depends(get_current_user), use_cases: TrainingUseCases=Depends(get_use_cases)):
