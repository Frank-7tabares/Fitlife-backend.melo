...
async def list_users(_admin: User=Depends(get_current_admin), user_repo: SQLAlchemyUserRepository=Depends(get_user_repository)):
...
async def update_user(user_id: UUID, request: AdminUserUpdateRequest, admin: User=Depends(get_current_admin), user_repo: SQLAlchemyUserRepository=Depends(get_user_repository)):
    ...
    return AdminUserListItem(id=updated.id, email=updated.email, full_name=updated.full_name, role=updated.role, is_active=updated.is_active, created_at=updated.created_at)

@router.delete('/users/{user_id}', status_code=status.HTTP_204_NO_CONTENT)
async def delete_user(user_id: UUID, admin: User=Depends(get_current_admin), db: AsyncSession=Depends(get_db)):
    if user_id == admin.id:
...
async def verify_instructor_certificate(instructor_id: UUID, request: VerifyCertificateRequest, _admin: User=Depends(get_current_admin), use_cases: InstructorUseCases=Depends(get_instructor_use_cases)):
    try:
...
async def upload_certificate(request: Request, file: UploadFile=File(...), _admin: User=Depends(get_current_admin)):
    if not file.filename:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail='Nombre de archivo vacío')
    ext = Path(file.filename).suffix.lower()
    if ext not in ALLOWED_EXTENSIONS:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Formato no permitido. Use: {', '.join(ALLOWED_EXTENSIONS)}")
