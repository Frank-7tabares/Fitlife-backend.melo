...
async def get_refresh_token_repository(db: AsyncSession=Depends(get_db)) -> SQLAlchemyRefreshTokenRepository:
    return SQLAlchemyRefreshTokenRepository(db)

async def get_current_user(credentials: HTTPAuthorizationCredentials=Depends(_bearer_scheme), user_repo: SQLAlchemyUserRepository=Depends(get_user_repository)) -> User:
    if not credentials:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail='Token de autenticación requerido')
    try:
...
async def get_current_user_optional(credentials: Optional[HTTPAuthorizationCredentials]=Depends(_bearer_scheme), user_repo: SQLAlchemyUserRepository=Depends(get_user_repository)) -> Optional[User]:
    if not credentials:
        return None
    try:
        payload = JWTService.decode_token(credentials.credentials)
...
async def get_current_instructor(current_user: User=Depends(get_current_user)) -> User:
    if current_user.role not in (UserRole.INSTRUCTOR, UserRole.ADMIN):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail='Acceso restringido a instructores')
    return current_user
