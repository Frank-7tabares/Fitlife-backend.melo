...
class TrainingUseCases:
    ...
    async def create_routine(self, request: CreateRoutineRequest) -> RoutineResponse:
        routine = Routine(id=uuid4(), name=request.name, description=request.description, goal=request.goal, level=request.level, exercises=[RoutineExercise(exercise_id=e.exercise_id, sets=e.sets, reps=e.reps, rest_seconds=e.rest_seconds) for e in request.exercises], creator_id=request.creator_id)
    ...
    async def update_routine(self, routine_id: UUID, user_id: UUID, request: UpdateRoutineRequest, is_admin: bool=False) -> RoutineResponse:
        ...
        updated = Routine(id=routine.id, name=request.name, description=request.description, goal=request.goal, level=request.level, exercises=[RoutineExercise(exercise_id=e.exercise_id, sets=e.sets, reps=e.reps, rest_seconds=e.rest_seconds) for e in request.exercises], creator_id=routine.creator_id)
    ...
    async def assign_routine(self, user_id: UUID, request: AssignRoutineRequest) -> bool:
        assignment = RoutineAssignment(user_id=user_id, routine_id=request.routine_id, assigned_at=datetime.utcnow(), is_active=True)
    ...
    async def complete_workout(self, user_id: UUID, request: CompleteWorkoutRequest) -> bool:
        ...
        completion = WorkoutCompletion(id=uuid4(), user_id=user_id, routine_id=assignment.routine_id, completed_at=datetime.utcnow(), effort_level=request.effort_level, notes=request.notes)
    ...
    async def get_completions(self, user_id: UUID, limit: int=50) -> List[WorkoutCompletionResponse]:
        ...
        return [WorkoutCompletionResponse(id=c.id, user_id=c.user_id, routine_id=c.routine_id, completed_at=c.completed_at, effort_level=c.effort_level, notes=c.notes) for c in completions]

    def _map_to_response(self, r: Routine, exercise_names: Optional[Dict[UUID, str]]=None) -> RoutineResponse:
        names = exercise_names or {}
        return RoutineResponse(id=r.id, name=r.name, description=r.description, goal=r.goal, level=r.level, exercises=[RoutineExerciseDTO(exercise_id=e.exercise_id, exercise_name=names.get(e.exercise_id), sets=e.sets, reps=e.reps, rest_seconds=e.rest_seconds) for e in r.exercises], creator_id=r.creator_id)
