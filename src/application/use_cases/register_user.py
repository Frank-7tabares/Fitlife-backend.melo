...
class RegisterUser:
    ...
    def __init__(self, user_repository: UserRepository, refresh_token_repository: SQLAlchemyRefreshTokenRepository, instructor_repository: SQLAlchemyInstructorRepository | None=None):
    ...
    async def execute(self, request: RegisterUserRequest) -> RegisterResponse:
        ...
        user = User(id=user_id, email=email, password_hash=PasswordHasher.hash(request.password), role=role, is_active=True, created_at=datetime.utcnow(), full_name=request.full_name, age=user_age)
        ...
        if role == UserRole.INSTRUCTOR and self.instructor_repository:
            ...
            instructor = Instructor(id=user_id, name=request.full_name or request.email, certifications=[], specializations=specs, rating_avg=0.0, active_users_count=0)
        ...
        return RegisterResponse(id=user.id, email=user.email, role=user.role, full_name=user.full_name, age=user.age, created_at=user.created_at, access_token=access_token, refresh_token=refresh_token)
