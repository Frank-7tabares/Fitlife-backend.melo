...
from ..dtos.physical_record_dtos import PhysicalRecordListResponse, PhysicalRecordRequest, PhysicalRecordResponse
...
class PhysicalRecordUseCases:
    ...
    async def create_record(self, user_id: UUID, request: PhysicalRecordRequest) -> PhysicalRecordResponse:
        ...
        record = PhysicalRecord(id=uuid4(), user_id=user_id, weight=request.weight, height=request.height, body_fat_percentage=request.body_fat_percentage, waist=request.waist, hip=request.hip, activity_level=request.activity_level, recorded_at=recorded_at)
    ...
    def _map_to_response(r: PhysicalRecord) -> PhysicalRecordResponse:
        return PhysicalRecordResponse(id=r.id, user_id=r.user_id, weight=r.weight, height=r.height, bmi=round(r.bmi, 2), body_fat_percentage=r.body_fat_percentage, waist=r.waist, hip=r.hip, activity_level=r.activity_level, recorded_at=r.recorded_at)
