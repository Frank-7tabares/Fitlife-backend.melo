"""Casos de uso para mensajería (Historia 7 - Mensajería, chat atleta-instructor)."""
...
async def resolve_coach_user_id_for_profile(
    profile_instructor_id: UUID,
...
async def allowed_coach_ids_for_assignment(
    profile_instructor_id: UUID,
    user_repo: UserRepository,
    instructor_repo: SQLAlchemyInstructorRepository,
) -> set[UUID]:
    uid = await resolve_coach_user_id_for_profile(
        profile_instructor_id, user_repo, instructor_repo
    )
    return {profile_instructor_id, uid}


async def coach_owns_assignment(
    profile_instructor_id: UUID,
    coach_user_id: UUID,
    user_repo: UserRepository,
    instructor_repo: SQLAlchemyInstructorRepository,
) -> bool:
    """True si `coach_user_id` es el instructor de esa asignación (perfil y/o usuario resuelto)."""
...
async def assert_can_converse_async(
    current_user: User,
    peer_id: UUID,
    user_repo: UserRepository,
    instructor_repo: SQLAlchemyInstructorRepository,
    ...
    if current_user.role == UserRole.USER:
        ...
        # Coach puede abrir chat con cualquier atleta (la UI lista /users/clients).
...
class SendMessage:
    """Envía mensaje entre atleta e instructor (y notificaciones del sistema)."""
    ...
    async def execute(self, sender_id: UUID, request: SendMessageRequest) -> MessageResponse:
        ...
        if request.message_type == MessageType.INSTRUCTOR_MESSAGE:
            ...
            if not assignment:
                # Fallback: si no hay asignación activa en BD, permitimos escribir al
                # coach SOLO si ya te envió mensajes antes (para evitar spam).
...
class GetMessagesByUser:
    ...
    async def execute(self, user_id: UUID, skip: int = 0, limit: int = 50) -> MessageListResponse:
        ...
        return MessageListResponse(
            ...
            messages=[self._map_to_response(msg) for msg in messages],
        )
...
class GetConversation:
    """Historial de chat entre dos usuarios enlazados por asignación (o admin)."""
    ...
    async def execute(self, current_user: User, peer_id: UUID) -> ConversationResponse:
        ...
        if current_user.role == UserRole.USER:
            ...
            if asn:
                coach_ids = await allowed_coach_ids_for_assignment(
                    asn.instructor_id, self.user_repo, self.instructor_repo
                )
                msgs = await self.message_repo.get_conversation_with_peers(
                    current_user.id, list(coach_ids)
                )
            else:
        ...
        return ConversationResponse(
            peer_id=peer_id,
            messages=[self._map_to_response(m) for m in msgs],
        )
...
class MarkThreadRead:
    """Marca como leídos los mensajes recibidos del peer hacia el usuario actual."""
        ...
        if current_user.role == UserRole.USER:
            ...
            if asn:
                ...
                for cid in coach_ids:
                    await self.message_repo.mark_thread_read_for_recipient(
                        current_user.id, cid
                    )
            else:
                # Sin asignación activa: marcamos como leídos todos los remitentes
                # que te han enviado mensajes no leídos (para que la campanita baje
                # aunque haya varios coaches sin fila en `instructor_assignments`).
