...
def demo_auth(client: httpx.Client) -> tuple[str, str]:
    ...
    user_id = None
    response = client.post(f'{BASE_URL}/auth/register', json={'email': 'carlos.perez@fitlife.com', 'password': 'MiPassword123!', 'full_name': 'Carlos Pérez'})
    user_data = print_response('Registro de usuario', response)
    if response.status_code == 201:
        user_id = user_data.get('id')
    elif response.status_code == 400 and 'already' in response.text.lower():
        print('  → Usuario ya existe, procediendo con login...')
    response = client.post(f'{BASE_URL}/auth/login', json={'email': 'carlos.perez@fitlife.com', 'password': 'MiPassword123!'})
...
def demo_profile(client: httpx.Client, user_id: str, token: str) -> None:
    ...
    response = client.patch(f'{BASE_URL}/users/{user_id}/profile', headers=headers, json={'full_name': 'Carlos A. Pérez', 'gender': 'male', 'age': 28, 'height': 178.0, 'fitness_goal': 'Ganar masa muscular', 'activity_level': 'MODERATE', 'version': current_version})
...
def demo_assessment(client: httpx.Client, user_id: str, token: str) -> None:
    ...
    headers = {'Authorization': f'Bearer {token}'}
    response = client.post(f'{BASE_URL}/assessments', headers=headers, json={'user_id': user_id, 'real_age': 28, 'responses': {'pushups': 20, 'run_minutes': 8, 'flexibility': 7, 'sleep_hours': 7, 'stress_level': 4}})
    ...
    response = client.get(f'{BASE_URL}/assessments/user/{user_id}', headers=headers)
...
def demo_physical_records(client: httpx.Client, user_id: str, token: str) -> None:
    ...
    response = client.post(url, headers=headers, json={'weight': 75.5, 'height': 178.0, 'body_fat_percentage': 17.0, 'waist': 82.0, 'hip': 95.0, 'activity_level': 'MODERATE'})
    record = print_response('Registro inicial', response)
    if response.status_code == 201:
        print(f"\n  IMC calculado automáticamente: {record.get('bmi', 'N/A'):.2f}")
    response = client.post(url, headers=headers, json={'weight': 74.8, 'height': 178.0, 'body_fat_percentage': 16.5, 'waist': 81.0, 'hip': 94.5, 'activity_level': 'ACTIVE'})
...
def demo_training(client: httpx.Client, user_id: str, token: str) -> None:
    ...
    response = client.post(f'{BASE_URL}/training/routines', headers=headers, json={'name': 'Rutina Fuerza Principiante', 'description': '3 días de entrenamiento de cuerpo completo para construir fuerza base', 'goal': 'Desarrollar fuerza y masa muscular inicial', 'level': 'BEGINNER', 'creator_id': user_id, 'exercises': [{'exercise_id': fake_exercise_id, 'sets': 3, 'reps': 12, 'rest_seconds': 60}, {'exercise_id': fake_exercise_id, 'sets': 4, 'reps': 8, 'rest_seconds': 90}]})
    routine = print_response('Crear rutina', response)
    if response.status_code != 201:
        print('  → No se pudo crear la rutina, saltando pasos de entrenamiento.')
        return
    routine_id = routine['id']
    response = client.post(f'{BASE_URL}/users/{user_id}/routines/assign', headers=headers, json={'routine_id': routine_id})
    print_response('Asignar rutina al usuario', response)
    response = client.get(f'{BASE_URL}/users/{user_id}/routines/active', headers=headers)
    print_response('Rutina activa del usuario', response)
    response = client.post(f'{BASE_URL}/workouts/complete', headers=headers, params={'user_id': user_id}, json={'effort_level': 7, 'notes': 'Aumenté el peso en press banca. Me sentí muy bien con la rutina.'})
...
def demo_nutrition(client: httpx.Client, user_id: str, token: str) -> None:
    ...
    headers = {'Authorization': f'Bearer {token}'}
    response = client.post(f'{BASE_URL}/nutrition/plans', headers=headers, params={'user_id': user_id}, json={'name': 'Plan Hiperproteico - Semana 9', 'description': 'Plan con superávit calórico para ganancia muscular limpia', 'week_number': 9, 'year': 2026, 'is_active': True, 'daily_plans': [{'day_of_week': 0, 'meals': [{'name': 'Desayuno', 'description': 'Avena con plátano y proteína en polvo', 'calories': 550, 'protein': 38.0, 'carbs': 72.0, 'fats': 10.0}, {'name': 'Almuerzo', 'description': 'Pechuga de pollo con arroz integral y brócoli', 'calories': 620, 'protein': 52.0, 'carbs': 65.0, 'fats': 12.0}, {'name': 'Cena', 'description': 'Salmón al horno con batata y espinacas', 'calories': 580, 'protein': 45.0, 'carbs': 50.0, 'fats': 18.0}]}, {'day_of_week': 1, 'meals': [{'name': 'Desayuno', 'description': 'Huevos revueltos con aguacate y pan integral', 'calories': 480, 'protein': 28.0, 'carbs': 38.0, 'fats': 22.0}, {'name': 'Almuerzo', 'description': 'Atún con pasta integral y tomates cherry', 'calories': 590, 'protein': 48.0, 'carbs': 62.0, 'fats': 10.0}, {'name': 'Cena', 'description': 'Carne magra con ensalada verde y quinoa', 'calories': 560, 'protein': 42.0, 'carbs': 45.0, 'fats': 15.0}]}]})
    plan = print_response('Crear plan de nutrición', response)
    if response.status_code == 201:
        total_calories = sum((meal['calories'] for day in plan['daily_plans'] for meal in day['meals'] if meal.get('calories')))
        print(f'\n  Calorías totales planificadas (2 días): {total_calories} kcal')
        print(f'  Promedio diario: {total_calories / 2:.0f} kcal/día')
    response = client.get(f'{BASE_URL}/users/{user_id}/nutrition/active', headers=headers)
