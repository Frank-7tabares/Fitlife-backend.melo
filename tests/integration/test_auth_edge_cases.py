"""
Pruebas de integración — Autenticación (casos borde)
...
def existing_user(client):
    """Crea un usuario real para reutilizar en pruebas del módulo."""
...
class TestRegisterEdgeCases:
    ...
    def test_email_with_leading_spaces_is_normalized_or_rejected(self, client):
        # email-validator v2.x normaliza el email quitando espacios en lugar de rechazarlo.
        # El resultado es aceptado (201) o rechazado (422) según la versión/config.
        response = client.post("/auth/register", json={
            "email": "  espacios@example.com",
            "password": "Password123!"
        })
        # 201: normalizado y registrado, 400/409: normalizado pero ya existe, 422: rechazado
        assert response.status_code in (201, 400, 409, 422)

    def test_register_without_full_name_is_allowed(self, client):
        """full_name es opcional — debe funcionar sin él."""
        response = client.post("/auth/register", json={
            "email": f"nofullname_{uuid.uuid4().hex[:6]}@example.com",
            "password": "Password123!"
        })
        assert response.status_code == 201
    ...
    def test_register_returns_correct_role(self, client):
        """Todos los usuarios nuevos deben tener rol USER."""
        response = client.post("/auth/register", json={
            "email": f"role_{uuid.uuid4().hex[:6]}@example.com",
            "password": "Password123!"
        })
        assert response.status_code == 201
    ...
    def test_register_does_not_return_password(self, client):
        """La respuesta nunca debe exponer la contraseña."""
        response = client.post("/auth/register", json={
            "email": f"secure_{uuid.uuid4().hex[:6]}@example.com",
            "password": "Password123!"
        })
        assert response.status_code == 201
        data = response.json()
...
class TestLoginEdgeCases:
    ...
    def test_empty_password_returns_401(self, client, existing_user):
        """Una contraseña vacía no coincide con el hash registrado."""
        response = client.post("/auth/login", json={
            "email": existing_user["email"],
            "password": ""
        })
    ...
    def test_case_sensitive_email(self, client, existing_user):
        """El email en mayúsculas no debe autenticar si se registró en minúsculas."""
        import pytest
        ...
        # Dependiendo de la BD/collation puede ser 401 o 200; no debe ser 500
