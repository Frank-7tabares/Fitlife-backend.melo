"""
Pruebas de integración — Evaluación física y registros físicos (casos borde)
...
# ---------------------------------------------------------------------------
# Evaluación física — casos borde
# ---------------------------------------------------------------------------

class TestAssessmentEdgeCases:

    def test_empty_responses_returns_score_zero(self, client, auth):
        """Evaluación sin preguntas → puntuación 0, categoría POOR."""
        response = client.post("/assessments", headers=auth["headers"], json={
    ...
    def test_all_max_responses_returns_excellent(self, client, auth):
        """Respuestas máximas → categoría EXCELLENT."""
        response = client.post("/assessments", headers=auth["headers"], json={
            "user_id": auth["user_id"],
            "real_age": 25,
            "responses": {f"q{i}": 10 for i in range(10)}
        })
        assert response.status_code == 201
        data = response.json()
    ...
    def test_body_age_younger_with_high_score(self, client, auth):
        """Score alto → edad corporal menor que la real."""
        response = client.post("/assessments", headers=auth["headers"], json={
            "user_id": auth["user_id"],
            "real_age": 30,
            "responses": {f"q{i}": 10 for i in range(10)}
    ...
    def test_body_age_older_with_low_score(self, client, auth):
        """Score bajo → edad corporal mayor que la real."""
        response = client.post("/assessments", headers=auth["headers"], json={
            "user_id": auth["user_id"],
            "real_age": 30,
    ...
    def test_assessment_history_empty_initially(self, client):
        """Un usuario nuevo sin evaluaciones retorna lista vacía."""
        import pytest
    ...
    def test_assessment_response_contains_required_fields(self, client, auth):
        response = client.post("/assessments", headers=auth["headers"], json={
            "user_id": auth["user_id"],
            "real_age": 28,
            "responses": {"pushups": 8, "flexibility": 6}
        })
        assert response.status_code == 201
        data = response.json()
...
# ---------------------------------------------------------------------------
# Registros físicos — casos borde
# ---------------------------------------------------------------------------

class TestPhysicalRecordEdgeCases:

    def test_weight_zero_returns_422(self, client, auth):
        """Peso igual a 0 viola la regla gt=0."""
        response = client.post(f"/users/{auth['user_id']}/physical-records",
    ...
    def test_height_zero_returns_422(self, client, auth):
        """Altura igual a 0 viola la regla gt=0."""
        response = client.post(f"/users/{auth['user_id']}/physical-records",
                               headers=auth["headers"], json={
            "weight": 70.0,
            "height": 0,
    ...
    def test_body_fat_above_100_returns_422(self, client, auth):
        """Porcentaje de grasa > 100 es inválido."""
        response = client.post(f"/users/{auth['user_id']}/physical-records",
                               headers=auth["headers"], json={
            "weight": 70.0,
            "height": 170.0,
            "body_fat_percentage": 101.0,
            "activity_level": "MODERATE"
        })
        assert response.status_code == 422

    def test_body_fat_negative_returns_422(self, client, auth):
        """Porcentaje de grasa negativo es inválido."""
        response = client.post(f"/users/{auth['user_id']}/physical-records",
                               headers=auth["headers"], json={
            "weight": 70.0,
            "height": 170.0,
            "body_fat_percentage": -1.0,
            "activity_level": "MODERATE"
        })
        assert response.status_code == 422

    def test_body_fat_exactly_70_is_valid(self, client, auth):
        """70% de grasa corporal es el valor límite superior permitido (rango realista RF-067)."""
        response = client.post(f"/users/{auth['user_id']}/physical-records",
                               headers=auth["headers"], json={
            "weight": 70.0,
            "height": 170.0,
            "body_fat_percentage": 70.0,
            "activity_level": "SEDENTARY"
        })
        assert response.status_code == 201

    def test_body_fat_above_70_returns_422(self, client, auth):
        """Porcentaje de grasa > 70 supera el rango realista (RF-067)."""
        response = client.post(f"/users/{auth['user_id']}/physical-records",
                               headers=auth["headers"], json={
            "weight": 70.0,
            "height": 170.0,
            "body_fat_percentage": 71.0,
            "activity_level": "SEDENTARY"
        })
        assert response.status_code == 422

    def test_body_fat_exactly_2_is_valid(self, client, auth):
        """2% de grasa corporal es el límite inferior permitido (rango realista RF-067)."""
        response = client.post(f"/users/{auth['user_id']}/physical-records",
                               headers=auth["headers"], json={
            "weight": 70.0,
            "height": 170.0,
            "body_fat_percentage": 2.0,
            "activity_level": "ACTIVE"
        })
        assert response.status_code == 201

    def test_body_fat_below_2_returns_422(self, client, auth):
        """Menos del 2% de grasa supera el rango realista inferior (RF-067)."""
        response = client.post(f"/users/{auth['user_id']}/physical-records",
                               headers=auth["headers"], json={
            "weight": 70.0,
            "height": 170.0,
    ...
    def test_bmi_is_calculated_automatically(self, client, auth):
        """El BMI debe calcularse como weight/(height_m)^2."""
        response = client.post(f"/users/{auth['user_id']}/physical-records",
                               headers=auth["headers"], json={
            "weight": 70.0,
            "height": 175.0,
    ...
    def test_optional_fields_can_be_omitted(self, client, auth):
        """waist, hip y body_fat_percentage son opcionales."""
        response = client.post(f"/users/{auth['user_id']}/physical-records",
                               headers=auth["headers"], json={
            "weight": 75.0,
            "height": 180.0,
    ...
    def test_history_returns_paginated_response(self, client, auth):
        """El historial devuelve PhysicalRecordListResponse con 'records' y 'total' (RF-065)."""
    ...
    def test_invalid_user_id_in_path_returns_422(self, client, auth):
        response = client.post("/users/no-es-uuid/physical-records",
                               headers=auth["headers"], json={
            "weight": 70.0,
            "height": 170.0,
            "activity_level": "MODERATE"
    ...
    def test_weight_above_500_returns_422(self, client, auth):
        """Peso > 500 kg supera el rango realista (RF-067)."""
        response = client.post(f"/users/{auth['user_id']}/physical-records",
                               headers=auth["headers"], json={
            "weight": 501.0,
            "height": 170.0,
    ...
    def test_height_below_50_returns_422(self, client, auth):
        """Altura < 50 cm supera el rango realista (RF-067)."""
        response = client.post(f"/users/{auth['user_id']}/physical-records",
                               headers=auth["headers"], json={
            "weight": 70.0,
            "height": 49.0,
    ...
    def test_height_above_300_returns_422(self, client, auth):
        """Altura > 300 cm supera el rango realista (RF-067)."""
        response = client.post(f"/users/{auth['user_id']}/physical-records",
                               headers=auth["headers"], json={
            "weight": 70.0,
            "height": 301.0,
    ...
    def test_invalid_activity_level_returns_422(self, client, auth):
        """activity_level debe ser uno de los valores del enum (RF-061)."""
        response = client.post(f"/users/{auth['user_id']}/physical-records",
                               headers=auth["headers"], json={
            "weight": 70.0,
            "height": 170.0,
    ...
    def test_all_activity_levels_are_valid(self, client, auth):
        """Todos los valores del enum ActivityLevel deben ser aceptados (RF-061)."""
    ...
    def test_recorded_at_can_be_provided(self, client, auth):
        """El usuario puede proporcionar recorded_at para registrar mediciones pasadas (RF-064)."""
        response = client.post(f"/users/{auth['user_id']}/physical-records",
                               headers=auth["headers"], json={
            "weight": 70.0,
            "height": 170.0,
    ...
    def test_recorded_at_defaults_to_now_when_omitted(self, client, auth):
        """Si no se provee recorded_at, el sistema lo genera automáticamente (RF-064)."""
        response = client.post(f"/users/{auth['user_id']}/physical-records",
                               headers=auth["headers"], json={
            "weight": 70.0,
            "height": 170.0,
