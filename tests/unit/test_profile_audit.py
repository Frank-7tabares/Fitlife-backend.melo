...
class TestGetProfileAudit:
    ...
    async def test_get_audit_history_success(self):
        user_id = uuid4()
        user = User(id=user_id, email='test@example.com', password_hash='hash', role=UserRole.USER, is_active=True, created_at=datetime.utcnow())
        audit_logs = [ProfileAuditLog(id=uuid4(), user_id=user_id, changed_by=user_id, changes={'full_name': {'old': None, 'new': 'John Doe'}}, timestamp=datetime.utcnow()), ProfileAuditLog(id=uuid4(), user_id=user_id, changed_by=user_id, changes={'age': {'old': None, 'new': 30}}, timestamp=datetime.utcnow())]
        user_repo = AsyncMock()
    ...
    async def test_get_audit_history_multiple_field_changes(self):
        user_id = uuid4()
        user = User(id=user_id, email='test@example.com', password_hash='hash', role=UserRole.USER, is_active=True, created_at=datetime.utcnow())
        audit_log = ProfileAuditLog(id=uuid4(), user_id=user_id, changed_by=user_id, changes={'full_name': {'old': 'John', 'new': 'John Doe'}, 'age': {'old': 25, 'new': 30}, 'gender': {'old': None, 'new': 'MALE'}, 'height': {'old': 175.0, 'new': 180.0}, 'fitness_goal': {'old': None, 'new': 'WEIGHT_LOSS'}}, timestamp=datetime.utcnow())
        user_repo = AsyncMock()
        user_repo.find_by_id.return_value = user
        audit_repo = AsyncMock()
