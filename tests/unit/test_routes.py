"""
Tests unitarios — Rutas de la API (adaptadores FastAPI).
Usa TestClient con dependency_overrides. Fixtures y factories en conftest/helpers.
"""
import pytest
...
from tests.unit.helpers import (
    make_user_entity,
    make_assessment_entity,
    make_physical_record_entity,
    make_routine_entity,
    make_nutrition_plan_entity,
)
...
class TestAuthRoutes:
    @pytest.mark.parametrize("exists_email,payload,expected_status,check_detail", [
        (False, {"email": "new@test.com", "password": "StrongPass1!", "full_name": "New User"}, 201, "email"),
        (True, {"email": "dup@test.com", "password": "StrongPass1!"}, 409, "already"),
    ])
...
class TestUserRoutes:
    ...
    def test_update_profile_success(self, app_with_mock_db):
        ...
        app.dependency_overrides[get_repos] = override_repos
        with TestClient(app) as client:
            resp = client.patch(f"/api/v1/users/{uid}/profile", json={
                "full_name": "New Name", "version": 1
            })
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_update_profile_not_found_returns_400(self, app_with_mock_db):
        ...
        app.dependency_overrides[get_repos] = override_repos
        with TestClient(app) as client:
            resp = client.patch(f"/api/v1/users/{uuid4()}/profile", json={
...
class TestAssessmentRoutes:
    def test_submit_assessment_success(self, app_with_mock_db):
        ...
        with TestClient(app) as client:
            resp = client.post("/api/v1/assessments", json={
                "user_id": str(uid), "real_age": 30, "responses": {"q1": 8, "q2": 7}
            })
    ...
    @pytest.mark.parametrize("has_data,expected_len,check_disclaimer", [
        (False, 0, False),
        (True, 1, True),
    ])
    ...
    def test_get_user_assessments_alias(self, app_with_mock_db, has_data):
        """GET /users/{id}/assessments (alias) vacío o con lista."""
        app, _ = app_with_mock_db
        from src.adapters.api.routes.assessment_routes import get_assessment_repository
        uid = uuid4()
        mock_repo = AsyncMock()
...
class TestInstructorRoutes:
    ...
    def test_list_instructors_returns_items(self, app_with_mock_db):
        ...
        mock_uc = AsyncMock()
        mock_uc.list_instructors.return_value = [
            InstructorResponse(id=iid, name="Coach", certifications=["ACE"], specializations="Fuerza", rating_avg=4.5, active_users_count=2)
        ]
    ...
    def test_create_instructor_returns_201(self, app_with_mock_db):
        ...
        iid = uuid4()
        mock_uc = AsyncMock()
        mock_uc.create_instructor.return_value = InstructorResponse(
            id=iid, name="New Coach", certifications=["ACE"], specializations="Cardio", rating_avg=0.0, active_users_count=0
        )
...
class TestPhysicalRecordRoutes:
    def test_create_record_success(self, app_with_mock_db):
        ...
        with TestClient(app) as client:
            resp = client.post(f"/api/v1/users/{uid}/physical-records", json={
                "weight": 75.0, "height": 175.0, "activity_level": "MODERATE"
            })
        assert resp.status_code == 201
        data = resp.json()
...
class TestTrainingRoutes:
    def test_create_routine_success(self, app_with_mock_db):
        ...
        with TestClient(app) as client:
            resp = client.post("/api/v1/training/routines", json={
                "name": "Full Body",
                "description": "All muscles",
                "goal": "STRENGTH",
                "level": "BEGINNER",
                "exercises": [{"exercise_id": str(exercise_id), "sets": 3, "reps": 10, "rest_seconds": 60}],
    ...
    def test_assign_routine_success(self, app_with_mock_db):
        ...
        app.dependency_overrides[get_use_cases] = override_uc
        with TestClient(app) as client:
            resp = client.post(f"/api/v1/users/{uid}/routines/assign", json={
    ...
    def test_complete_workout_no_assignment_returns_400(self, app_with_mock_db):
        ...
        app.dependency_overrides[get_use_cases] = override_uc
        with TestClient(app) as client:
            resp = client.post(f"/api/v1/workouts/complete", params={"user_id": str(uid)}, json={
                "effort_level": 7
            })
        assert resp.status_code == 400
...
class TestNutritionRoutes:
    def test_create_nutrition_plan_success(self, app_with_mock_db):
        ...
        with TestClient(app) as client:
            resp = client.post("/api/v1/nutrition/plans", params={"user_id": str(uid)}, json={
                "name": "Week 1",
