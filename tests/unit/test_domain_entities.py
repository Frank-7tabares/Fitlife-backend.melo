...
class TestUserEntity:
    ...
    def _make_user(self, **kwargs):
        ...
        defaults = dict(id=uuid4(), email='test@example.com', password_hash='hashed', role=UserRole.USER, is_active=True, created_at=datetime.utcnow())
...
class TestPhysicalRecordBmi:
    ...
    def _make_record(self, weight: float, height: float, **kwargs):
        ...
        defaults = dict(id=uuid4(), user_id=uuid4(), body_fat_percentage=None, waist=None, hip=None, activity_level=ActivityLevel.MODERATE, recorded_at=datetime.utcnow())
        defaults.update(kwargs)
        return PhysicalRecord(weight=weight, height=height, **defaults)

    @pytest.mark.parametrize('weight,height,expected_bmi', [(70.0, 175.0, 70.0 / 1.75 ** 2), (80.0, 180.0, 80 / 1.8 ** 2), (75.0, 170.0, 75.0 / 1.7 ** 2)])
    def test_bmi_formula(self, weight, height, expected_bmi):
        record = self._make_record(weight, height)
        assert record.bmi == pytest.approx(expected_bmi, rel=1e-06)

    @pytest.mark.parametrize('weight,height,predicate', [(45.0, 170.0, lambda b: b < 18.5), (65.0, 170.0, lambda b: 18.5 <= b < 25.0), (90.0, 170.0, lambda b: b >= 25.0)])
    ...
    def test_bmi_with_optional_fields(self):
        from src.domain.entities.physical_record import ActivityLevel, PhysicalRecord
        record = PhysicalRecord(id=uuid4(), user_id=uuid4(), weight=75.0, height=170.0, body_fat_percentage=20.5, waist=85.0, hip=95.0, activity_level=ActivityLevel.ACTIVE, recorded_at=datetime.utcnow())
        assert record.bmi == pytest.approx(75.0 / 1.7 ** 2, rel=1e-06)
...
class TestAssessmentEntity:
    ...
    def test_assessment_construction(self):
        ...
        uid = uuid4()
        now = datetime.utcnow()
        a = Assessment(id=uid, user_id=uuid4(), fitness_score=75.0, category=AssessmentCategory.GOOD, body_age=28.0, comparison=BodyAgeComparison.BODY_YOUNGER, responses={'q1': 7}, created_at=now)
...
class TestRoutineEntity:
    ...
    def test_routine_creation(self):
        ...
        r = Routine(id=uuid4(), name='Full Body', description='Full body workout', goal='STRENGTH', level=FitnessLevel.INTERMEDIATE, exercises=[RoutineExercise(exercise_id=uuid4(), sets=3, reps=12, rest_seconds=45)], creator_id=uuid4())
    ...
    def test_routine_empty_exercises(self):
        ...
        r = Routine(id=uuid4(), name='Rest', description='Rest day', goal='RECOVERY', level=FitnessLevel.BEGINNER, exercises=[], creator_id=uuid4())
        assert r.exercises == []
...
class TestRoutineAssignmentEntity:
    ...
    def test_routine_assignment_fields(self):
        ...
        now = datetime.utcnow()
        a = RoutineAssignment(user_id=uuid4(), routine_id=uuid4(), assigned_at=now, is_active=True)
        assert a.is_active is True
        assert a.assigned_at == now

    def test_routine_assignment_inactive(self):
        from src.domain.entities.training import RoutineAssignment
        a = RoutineAssignment(user_id=uuid4(), routine_id=uuid4(), assigned_at=datetime.utcnow(), is_active=False)
...
class TestWorkoutCompletionEntity:
    ...
    def test_workout_completion_fields(self):
        ...
        now = datetime.utcnow()
        wc = WorkoutCompletion(id=uuid4(), user_id=uuid4(), routine_id=uuid4(), completed_at=now, effort_level=8, notes='Felt great')
    ...
    def test_workout_completion_no_notes(self):
        from src.domain.entities.training import WorkoutCompletion
        wc = WorkoutCompletion(id=uuid4(), user_id=uuid4(), routine_id=uuid4(), completed_at=datetime.utcnow(), effort_level=5, notes=None)
...
class TestDailyPlanEntity:
    ...
    def test_daily_plan_construction(self):
        ...
        dp = DailyPlan(day_of_week=0, meals=[Meal('Eggs', 'Breakfast', 200, 20.0, 2.0, 10.0)])
...
class TestNutritionPlanEntity:
    ...
    def test_nutrition_plan_construction(self):
        ...
        now = datetime.utcnow()
        plan = NutritionPlan(id=uuid4(), user_id=uuid4(), name='Week 1 Plan', description='Bulking plan', week_number=1, year=2026, daily_plans=[DailyPlan(0, [Meal('Rice', 'Lunch', 500, 30.0, 80.0, 8.0)])], is_active=True, created_at=now)
    ...
    def test_nutrition_plan_inactive(self):
        ...
        plan = NutritionPlan(id=uuid4(), user_id=uuid4(), name='Old Plan', description='Outdated', week_number=50, year=2025, daily_plans=[], is_active=False, created_at=datetime.utcnow())
...
class TestProfileAuditLog:
    ...
    def test_audit_log_construction(self):
        ...
        now = datetime.utcnow()
        uid = uuid4()
        log = ProfileAuditLog(id=uuid4(), user_id=uid, changed_by=uid, changes={'full_name': {'old': 'Ana', 'new': 'Ana López'}}, timestamp=now)
...
class TestInstructorEntities:
    ...
    def test_instructor_construction(self):
        ...
        iid = uuid4()
        inst = Instructor(id=iid, name='Coach Ana', certifications=['ACE', 'NASM'], specializations='Fuerza', rating_avg=4.5, active_users_count=3)
        assert inst.id == iid
    ...
    def test_instructor_assignment_construction(self):
        ...
        uid, iid = (uuid4(), uuid4())
        now = datetime.utcnow()
        a = InstructorAssignment(id=uuid4(), user_id=uid, instructor_id=iid, started_at=now, ended_at=None, is_active=True)
    ...
    def test_instructor_rating_construction(self):
        ...
        r = InstructorRating(id=uuid4(), user_id=uuid4(), instructor_id=uuid4(), rating=4, created_at=datetime.utcnow(), comment='Muy bueno')
        assert r.rating == 4
