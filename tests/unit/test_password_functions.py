"""Tests para password reset, change y validación."""
import pytest
...
class TestPasswordResetRequest:
    """Tests para solicitud de reset de contraseña."""
    ...
    async def test_reset_request_user_not_exists(self):
        """Respuesta genérica si usuario no existe (prevenir enumeración)."""
        user_repo = AsyncMock()
        ...
        # Respuesta genérica sin revelar si existe
...
class TestPasswordReset:
    """Tests para confirmación de reset de contraseña."""
    ...
    async def test_reset_with_valid_token(self):
        ...
        user = User(
            id=user_id,
            email="test@example.com",
            password_hash=PasswordHasher.hash("OldPassword123!"),
            role=UserRole.USER,
            is_active=True,
            created_at=datetime.utcnow()
        )
        ...
        result = await use_case.execute(PasswordResetDto(
            token="valid_token_123",
    ...
    async def test_reset_with_expired_token(self):
        """Reset falla con token expirado."""
        token_id = uuid4()
        ...
        with pytest.raises(ValueError) as exc:
            await use_case.execute(PasswordResetDto(
                token="expired_token",
                new_password="NewPassword456!"
            ))
    ...
    async def test_reset_with_weak_password(self):
        """Reset falla si nueva contraseña es débil."""
        reset_token = PasswordResetToken(
            id=uuid4(),
            user_id=uuid4(),
            token="valid_token",
...
class TestPasswordChange:
    ...
    async def test_change_password_success(self):
        ...
        result = await use_case.execute(str(user_id), PasswordChangeDto(
    ...
    async def test_change_password_wrong_current(self):
        """Cambio falla si contraseña actual es incorrecta."""
        user_id = uuid4()
        ...
        with pytest.raises(ValueError) as exc:
            await use_case.execute(str(user_id), PasswordChangeDto(
    ...
    async def test_change_password_weak_new(self):
        """Cambio falla si nueva contraseña es débil."""
        user_id = uuid4()
        old_password = "OldPassword123!"
