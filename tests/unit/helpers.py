"""
Helpers compartidos para tests unitarios: factories de entidades y mocks.
"""
...
def make_routine_entity():
    ...
    return Routine(
        ...
        exercises=[RoutineExercise(exercise_id=uuid4(), sets=3, reps=10, rest_seconds=60)],
        creator_id=uuid4(),
    )
...
def make_nutrition_plan_entity(user_id=None):
    ...
    return NutritionPlan(
        ...
        week_number=1,
        year=2026,
        daily_plans=[DailyPlan(0, [Meal("Oats", "Breakfast", 300, 15.0, 50.0, 5.0)])],
        is_active=True,
        created_at=datetime.utcnow(),
    )
...
# Mock ORM models (para repositorios)
def mock_user_model(user_id=None, email="u@e.com", password_hash="h", role="USER",
                    is_active=True, full_name=None, created_at=None, updated_at=None, version=1):
    m = MagicMock()
...
def mock_nutrition_plan_model(plan_id=None, user_id=None):
    ...
    m.plans_data = [{
        "day_of_week": 0,
        "meals": [{"name": "Oats", "description": "Breakfast", "calories": 300, "protein": 15.0, "carbs": 50.0, "fats": 5.0}]
    }]
    m.is_active = True
    m.created_at = datetime.utcnow()
    return m
