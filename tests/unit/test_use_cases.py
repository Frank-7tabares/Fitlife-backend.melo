...
class TestRegisterUser:
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize('email,password,full_name,expected_full_name,expected_role', [('noname@example.com', 'Secret99!', None, None, 'USER'), ('x@x.com', 'Abcde123!', 'Name', 'Name', 'USER')])
...
class TestLoginUser:
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize('find_by_email_val,password,match_msg', [(None, 'any_pass', 'Invalid credentials'), ('wrong_pass', 'wrong_pass', 'Invalid credentials'), ('inactive', 'pass123', 'inactive')])
...
class TestUpdateProfile:
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize('full_name_req,version,expected_name,expect_update', [('Same Name', 1, 'Same Name', False), (None, 1, 'Existing', False)])
...
class TestSubmitAssessment:
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize('responses,real_age,expected_score,expected_category', [({}, 25, 0.0, 'POOR'), ({f'q{i}': 10 for i in range(5)}, 30, None, 'EXCELLENT')])
...
class TestGetAssessmentHistory:
    ...
    async def test_returns_mapped_with_disclaimer(self, repo):
        ...
        uid = uuid4()
        repo.find_by_user_id.return_value = [Assessment(id=uuid4(), user_id=uid, fitness_score=75.0, category=AssessmentCategory.GOOD, body_age=28.0, comparison=BodyAgeComparison.BODY_YOUNGER, responses={'q': 7}, created_at=datetime.utcnow()), Assessment(id=uuid4(), user_id=uid, fitness_score=40.0, category=AssessmentCategory.POOR, body_age=35.0, comparison=BodyAgeComparison.BODY_OLDER, responses={'q': 3}, created_at=datetime.utcnow())]
...
class TestPhysicalRecordUseCases:
    ...
    async def test_get_history_empty_and_mapped(self, repo):
        ...
        uid = uuid4()
        repo.find_by_user_id.return_value = [PhysicalRecord(id=uuid4(), user_id=uid, weight=70.0, height=170.0, body_fat_percentage=None, waist=None, hip=None, activity_level=ActivityLevel.LIGHT, recorded_at=datetime.utcnow())]
...
class TestTrainingUseCases:
    ...
    async def test_create_routine_returns_response(self, routine_repo, assignment_repo):
        ...
        req = CreateRoutineRequest(name='Upper Body', description='Chest and back', goal='HYPERTROPHY', level=FitnessLevel.INTERMEDIATE, exercises=[RoutineExerciseDTO(exercise_id=uuid4(), sets=3, reps=10, rest_seconds=60)], creator_id=uuid4())
...
class TestNutritionUseCases:
    ...
    async def test_create_plan_returns_response(self, repo):
        ...
        req = CreateNutritionPlanRequest(name='Cutting Plan', description='Reduce body fat', week_number=10, year=2026, daily_plans=[DailyPlanDTO(day_of_week=0, meals=[MealDTO(name='Oats', description='Breakfast', calories=300, protein=15.0, carbs=50.0, fats=5.0)])], is_active=True)
    ...
    async def test_create_plan_optional_macros_and_daily_plans(self, repo):
        from src.application.use_cases.nutrition_use_cases import NutritionUseCases
        from src.application.dtos.nutrition_dtos import CreateNutritionPlanRequest, DailyPlanDTO, MealDTO
        req = CreateNutritionPlanRequest(name='Basic', description='Simple', week_number=1, year=2026, daily_plans=[DailyPlanDTO(day_of_week=1, meals=[MealDTO(name='Apple', description='Snack')])], is_active=False)
        result = await NutritionUseCases(repo).create_plan(uuid4(), req)
    ...
    async def test_get_active_plan_none_and_found(self, repo):
        ...
        uid = uuid4()
        repo.find_active_by_user_id.return_value = NutritionPlan(id=uuid4(), user_id=uid, name='Active Plan', description='Current', week_number=5, year=2026, daily_plans=[DailyPlan(0, [Meal('Rice', 'Lunch', 500, 30.0, 80.0, 8.0)])], is_active=True, created_at=datetime.utcnow())
...
class TestInstructorUseCases:
    ...
    async def test_list_instructors_empty_and_with_items(self, use_cases, instructor_repo, assignment_repo):
        ...
        mid = uuid4()
        instructor_repo.find_all.return_value = [InstructorModel(id=str(mid), name='Ana', certifications=['ACE'], specializations='Fuerza', rating_avg=4.5, certificate_status='verified')]
    ...
    async def test_list_instructors_skips_non_verified_by_default(self, use_cases, instructor_repo, assignment_repo):
        from src.infrastructure.database.models.instructor_models import InstructorModel
        v, p = (uuid4(), uuid4())
        instructor_repo.find_all.return_value = [InstructorModel(id=str(v), name='Veri', certifications=[], specializations='', rating_avg=0.0, certificate_status='verified'), InstructorModel(id=str(p), name='Pend', certifications=[], specializations='', rating_avg=0.0, certificate_status='pending')]
    ...
    async def test_assign_instructor_success_and_not_found(self, use_cases, instructor_repo, assignment_repo):
        ...
        uid, iid = (uuid4(), uuid4())
        instructor_repo.find_by_id.return_value = InstructorModel(id=str(iid), name='Coach', certifications=[], specializations='', rating_avg=0.0, certificate_status='verified')
    ...
    async def test_assign_instructor_rejects_not_verified(self, use_cases, instructor_repo, assignment_repo):
        ...
        from src.infrastructure.database.models.instructor_models import InstructorModel
        uid, iid = (uuid4(), uuid4())
        instructor_repo.find_by_id.return_value = InstructorModel(id=str(iid), name='Coach', certifications=[], specializations='', rating_avg=0.0, certificate_status='pending')
