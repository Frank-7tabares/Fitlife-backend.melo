...
async def generate_users(session: AsyncSession) -> list:
    ...
    for _ in range(NUM_USERS):
        model = UserModel(id=str(uuid4()), email=fake.unique.email(), password_hash=password_hash, role='USER', is_active=True, full_name=fake.name(), created_at=fake.date_time_between(start_date='-6m', end_date='now'))
        session.add(model)
        users.append(model)
...
async def generate_assessments(session: AsyncSession, users: list) -> None:
    ...
    for _ in range(NUM_ASSESSMENTS):
        ...
        model = AssessmentModel(id=str(uuid4()), user_id=user.id, fitness_score=score, category=categories[min(3, int((100 - score) / 25))], body_age=body_age, comparison=comparisons[0] if body_age < real_age else comparisons[1], responses=json.dumps({'q1': random.randint(1, 10), 'q2': random.randint(1, 5)}), created_at=fake.date_time_between(start_date='-3m', end_date='now'))
        session.add(model)
    await session.commit()
